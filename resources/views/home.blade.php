                    ...
                    <div class="relative bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/50 dark:border-slate-800 p-6 z-10">
                        ...
                        <div class="space-y-4">
                            ...
                            @if(isset($featuredJobs) && count($featuredJobs) > 0)
                                @foreach(array_slice((is_array($featuredJobs) ? $featuredJobs : $featuredJobs->toArray()), 0, 2) as $index => $job)
                                @php
                                ...
                                <div class="flex items-center gap-4 p-4 rounded-2xl bg-white dark:bg-slate-800 shadow-sm border border-slate-100 dark:border-slate-700/50 transition-all hover:scale-[1.02]">
                                    <div class="w-12 h-12 rounded-xl {{ $color }} flex items-center justify-center text-white shrink-0 shadow-lg {{ $shadow }}">
                                        <i data-lucide="{{ $icon }}" class="w-6 h-6"></i>
                                    </div>
                                    <div class="flex-1">
                                        <div class="flex justify-between items-center mb-1">
                                            <p class="text-sm font-bold text-slate-800 dark:text-white line-clamp-1" title="{{ $cleanTitle }}">{{ $cleanTitle }}</p>
                                            @if($isRemote)
                                        ...
                                        <p class="text-[10px] text-slate-500 dark:text-slate-400 font-medium line-clamp-1" title="{{ $company }} • {{ $location }}">{{ $company }} &bull; {{ $location }}</p>
                                    </div>
                                </div>
                                @endforeach
                            @endif
            ...
            <div class="group bg-white dark:bg-slate-900 rounded-[24px] p-6 border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-xl hover:shadow-emerald-500/5 hover:border-emerald-200 dark:hover:border-emerald-800 transition-all duration-300 relative overflow-hidden flex flex-col justify-between">
                <div>
                    ...
                    <h3 class="text-xl font-bold text-slate-900 dark:text-white mb-2 group-hover:text-emerald-700 dark:group-hover:text-emerald-400 transition-colors line-clamp-2">
                        <a href="{{ route('jobs.detail', ['title' => $job->title, 'company' => $job->company, 'location' => $job->location_text ?: $job->location, 'url' => $job->url]) }}" class="before:absolute before:inset-0">{{ $cleanTitle }}</a>
                    </h3>
                    <p class="text-slate-500 dark:text-slate-400 mb-6 font-medium line-clamp-1">{{ $job->company }} • {{ $job->location_text ?: $job->location }}</p>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div class="group relative bg-slate-50 dark:bg-slate-800/50 rounded-[32px] p-8 border border-slate-100 dark:border-slate-700 overflow-hidden hover:border-emerald-200 dark:hover:border-emerald-700 transition-colors">
                ...
                <div class="relative z-10">
                    ...
                    <a href="{{ route('applications.index') }}" class="inline-flex items-center gap-2 font-semibold text-emerald-700 dark:text-emerald-400 hover:text-emerald-700 dark:hover:text-emerald-300 transition-colors">
