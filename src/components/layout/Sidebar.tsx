...
import { LayoutDashboard, Search, User, Bell, LogOut, Briefcase, ClipboardList } from "lucide-react";
                    ...
                    {navItems.map((item) => (
                        <li key={item.to}>
                            <NavLink
                                to={item.to}
                                className={({ isActive }) =>
                                    `flex items-center px-6 py-3 rounded-lg font-medium transition-colors duration-200 hover:bg-navy-50 hover:text-navy-700 ${isActive || location.pathname === item.to
