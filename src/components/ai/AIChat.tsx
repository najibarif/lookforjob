...
import { Send, User, Bot, FileText, Upload } from "lucide-react";
    ...
    return (
        <div className={`flex ${isUser ? "justify-end" : "justify-start"} mb-4`}>
            <div className={`flex max-w-[80%] ${isUser ? "flex-row-reverse" : ""}`}>
                <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${isUser ? "ml-2 bg-primary" : "mr-2 bg-accent"
                        }`}
                >
                    {isUser ? (
                ...
                <div
                    className={`p-3 rounded-2xl border-2 border-black ${isUser ? "bg-primary/10" : "bg-accent/10"
...
const AIChat = ({ cvText = null, className = "" }: AIChatProps) => {
    ...
    const [uploadedCV, setUploadedCV] = useState<string | null>(cvText || null);
